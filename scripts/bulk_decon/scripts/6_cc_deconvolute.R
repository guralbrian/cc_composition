...
all.markers <- read.csv("data/processed/single_cell/cc_cluster_markers.csv")

# Load whole bulk RNAseq
bulk.all <- read.csv("data/processed/bulk/cc_and_fractions.csv", row.names = 1, check.names = F)
...
# Merge composition data with phenotypes
colnames(phenotypes_real)[2] <- "Sub"
decon.whole <- decon.melt|> 
  #mutate(Sub = paste0("S", Sub)) |> 
  subset(Sub %in% phenotypes_real$Sub) |> 
  left_join(phenotypes_real) |>
  mutate(Treatment = factor(case_when(
      str_detect(Treatment, "Ctrl") ~ "Control",
      str_detect(Treatment, "Iso") ~ "Isoproterenol")))


#Save outputs
write.csv(decon.whole, "data/processed/compositions/cc_samples.csv", row.names = F)
write.csv(decon.frac, "data/processed/compositions/cc_fraction_samples.csv", row.names = F)
