...
# Load compositions
decon.whole <- read.csv("data/processed/compositions/cc_samples.csv")
decon.frac <- read.csv("data/processed/compositions/cc_fraction_samples.csv")
clusters <- read.csv("data/processed/bulk/cc_pca_clusters.csv") |>
  select(PC1, PC2, Adaptor, cluster)
colnames(clusters)[3] <- "Sub"

decon.whole <- left_join(decon.whole, clusters)
...
png(file = "results/7_plot_comps/cc_pure_cell_types.png",
    width = 1600, 
    height = 800,
    units = "px",
    res = 100)
...
#legend.names <- c("Control","Isoproterenol")
...
comp_celltype <- decon.whole   %>%
  ggplot(aes(x = factor(CellType_wrap, levels = as.character(cell.type.order)), y = Prop, fill = cluster)) +
...
png(file = "results/7_plot_comps/cc_cluster_comps.png",
    width = 1600, 
    height = 800,
    units = "px",
    res = 100)
