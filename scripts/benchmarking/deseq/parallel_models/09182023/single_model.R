...
counts.dict <- read.csv("results/benchmarking/deseq/comp_adj/parallel_models/09182023/input_data/counts_dict_2000g.csv")[start_row:end_row,]
counts.small <- read.csv("results/benchmarking/deseq/comp_adj/parallel_models/09182023/input_data/counts_2000g.csv", row.names = 1)[start_row:end_row,]
ratios <- read.csv("results/benchmarking/deseq/comp_adj/parallel_models/09182023/input_data/ratios_2000g.csv", row.names = 1)
...
models <- list(
  ...
  raw_cm_fb     = ~ 0 + pct.change + Cardiomyocytes + Monocyte__Myeloid ,
  raw_cm_fb_im  = ~ 0 + pct.change + Cardiomyocytes + Monocyte__Myeloid  + Valve_Cell__Mesen_ ,
  raw_4         = ~ 0 + pct.change + Cardiomyocytes + Monocyte__Myeloid  + Valve_Cell__Mesen_  + unclear,
  clr_cm        = ~ 0 + pct.change + clr.Cardiomyocytes,
  clr_cm_fb     = ~ 0 + pct.change + clr.Cardiomyocytes + clr.Monocyte__Myeloid ,
  clr_cm_fb_im  = ~ 0 + pct.change + clr.Cardiomyocytes + clr.Monocyte__Myeloid  + clr.Valve_Cell__Mesen_ ,
  clr_4         = ~ 0 + pct.change + clr.Cardiomyocytes + clr.Monocyte__Myeloid  + clr.Valve_Cell__Mesen_  + clr.unclear,
...
mainDir <- "results/benchmarking/deseq/comp_adj/parallel_models/09182023/model_outputs"
